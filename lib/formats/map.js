...
const makeMetaEntries = dictionary => dictionary.allTokens.map(x =>
  [`--${x.name}`,
    {
      value: x.value,
...
export const metaMapEs = {
  ...
  formatter({ file, dictionary }) {
    return [
      fileHeader({ file }),
      `export const tokensMeta = new Map(${JSON.stringify(makeMetaEntries(dictionary), null, 2)});`,
    ].join('\n');
  },
};
...
export const metaMapCjs = {
  ...
  formatter: (...args) =>
    metaMapEs
      .formatter(...args)
      .replace('export const tokensMeta', 'exports.tokensMeta'),
};
